...
The dashboard files live in dashboard/ and currently read dashboard/sample-records.json. This is a local-first UI, not a deployed web app. Future work can connect the dashboard to the clean JSON generated by node index.js export:json.
...
The local server serves the dashboard and exposes simple API routes for records, sales notes, quote requests, market signals, and reset. Dashboard input writes through the backend and persists to memory.json. CLI commands still work alongside the local dashboard.
